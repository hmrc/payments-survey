...
final class SurveyController @Inject() (
  ...
  def surveyJourney(id: SurveyJourneyId): Action[AnyContent] = actions.maybeSurveyJourneyAction(id) {
    implicit request =>
      Ok(
        surveyJourney(
          surveyForm,
  ...
  def showSurveyThanks(id: SurveyJourneyId): Action[AnyContent] = actions.maybeSurveyJourneyAction(id) {
    implicit request => Ok(surveyThanks())
  }
}
